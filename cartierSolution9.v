...
SHORT ::
  ...
  Commonly , the "lambda calculus" has the « cancellation » computation [  application ( abstraction f ) s  :=  substitution f s  ] , where [ f ] is some function and [ s ] is some element , but now in contrast the function [ f ] shall be viewed as parametrized-over the now-function [ s ] which is some section parametrizing the application .  In other words , given any grammatical form-morphism [ f ] of [  |-  ( E ~> F )  ] parametrized-over the parametrizator-arrow ( "span" ) [  project <o> subst  ] , whose sensible-decoding is some « formatted » function [  project* E  |-  subst* F  ] across the « formats » ( "pullbacks" ) from [  project* E  ] to [  subst* F  ] parametrized-over the (elements of the) parameter (functor) [ P ] , oneself shall :
